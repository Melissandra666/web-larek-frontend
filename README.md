...
```
items:changed - это событие инициируется при обновлении списка продуктов, что позволяет интерфейсу отразить изменения в доступных товарах.
modal:openModal -  это событие срабатывает, когда модальное окно открывается, предоставляя пользователю доступ к дополнительной информации или действиям.
modal:closeModal - данное событие происходит при закрытии модального окна, что возвращает пользователя к основному интерфейсу.
product:add - это событие инициируется, когда пользователь добавляет товар в корзину, что позволяет обновить состояние корзины.
product:delete - данное событие возникает при удалении товара из корзины, обеспечивая актуальность списка товаров.
basket:changed - событие, которое сигнализирует об изменениях в интерфейсе корзины, позволяя обновить отображаемую информацию о текущих товарах.
counter:changed - это событие связано с изменением счетчика товаров в корзине, что помогает пользователю увидеть актуальное количество добавленных товаров.
order:changed - событие, возникающее при любых изменениях в заказе, что позволяет интерфейсу отразить новую информацию о текущем состоянии заказа.
address:change - это событие инициируется, когда пользователь изменяет адрес, позволяя системе обновить соответствующие данные.
contacts:change - данное событие происходит при изменении контактной информации пользователя, что обеспечивает актуальность данных.
errors:changed - событие, сигнализирующее о возникновении ошибок в форме, позволяя пользователю увидеть проблемы и исправить их.
order:submit - это событие происходит при подтверждении формы оплаты, что запускает процесс обработки заказа.
contacts:submit - данное событие инициируется при подтверждении формы с контактными данными, позволяя системе сохранить или обработать введенную информацию.
payment:toggle - событие, которое срабатывает при смене способа оплаты, обновляя интерфейс и доступные варианты.
```
